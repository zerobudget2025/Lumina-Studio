...
const Header: React.FC<HeaderProps> = ({ isPro, onTogglePro, onChangeKey }) => {
  return (
      ...
      <div className="container mx-auto px-6 h-20 flex items-center justify-between max-w-7xl">
        ...
        <div className="flex items-center gap-3">
          <div className="hidden md:flex flex-col items-end mr-2">
            ...
            <span className={`text-[10px] font-black uppercase transition-colors ${isPro ? 'text-purple-400' : 'text-blue-400'}`}>
          ...
          {isPro && (
            <button 
              onClick={onChangeKey}
              className="p-2.5 rounded-2xl bg-slate-900 border border-slate-800 text-slate-500 hover:text-white hover:border-slate-700 transition-all shadow-xl"
              title="Change API Key / Project"
            >
          ...
          <button 
            onClick={onTogglePro}
            className={`px-6 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all duration-500 border-2 relative overflow-hidden group ${
              isPro 
                ? 'bg-purple-600 border-purple-500 text-white shadow-lg shadow-purple-500/20 scale-105 hover:bg-purple-500' 
                : 'bg-slate-900 border-slate-800 text-slate-400 hover:text-white hover:border-blue-500/50 hover:shadow-blue-500/10 shadow-lg'
            }`}
          >
            {!isPro && (
               <span className="absolute inset-0 bg-blue-500/5 animate-pulse"></span>
            )}
